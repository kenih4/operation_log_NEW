...
# 消すな将来用    df['C'] = df['C'].replace({'終了時': '終了 時', '終了後': '終了 後'},regex=True) # とりあえずテスト的。今のところ意味ない
    ...
    df.drop(df[df['C'].str.contains('プロファイル定時確認',
            case=False, na=False)].index, inplace=True)
    df.drop(df[df['C'].str.contains(
        'プロファイル確認', case=False, na=False)].index, inplace=True)
    df.drop(df[df['C'].str.contains(
    ...
    for index, item in df.iterrows():
        ...
        try:
            if ((item['DT'] - bf_itemDT).total_seconds() >= 0):
                #                print('OK: 時系列になってます。     ',  item['DT'],"    - ", bf_itemDT, "   =   ", (item['DT'] - bf_itemDT).total_seconds())
                pass
            else:
...
#       日付を跨いだ対処　　改修前は以下のようにしていた。ちょっと意味が分からない
